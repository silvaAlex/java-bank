...
public class AccountRepository {
        ...
        if(!accounts.isEmpty()){
            ...
            for(var p : pix) {
                if(pixInUse.contains(p)){
                    var message = String.format("O pix %s já está em uso.", p);
                    throw new PixInUseException(message);
                }
            }
        }

        var newAccount = new AccountWallet(pix, initialFunds);
    ...
    public void transferMoney(final String sourcePix,  final String targetPix, final long amount) {
        ...
        target.addMoney(source.reduceMoney(amount), source.getBankService(), message);
    }
